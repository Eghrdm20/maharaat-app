...
export default function CourseDetailsPage({
  ...
  const handleBuyCourse = async () => {
    try {
      ...
      const Pi = window.Pi
      if (!Pi?.createPayment) {
        setStatus("الدفع غير متاح. افتح التطبيق من داخل Pi Browser")
        return
      }
            ...
            {!hasAccess ? (
              ...
              >
                ...
                <button
                  ...
                  disabled={buying || (!course.is_free && !piUser)}
                  style={{
                    border: "none",
                    background:
                      buying || (!course.is_free && !piUser) ? "#94a3b8" : "#0f172a",
                    color: "white",
                    borderRadius: 18,
                    padding: "16px 20px",
                    fontSize: 18,
                    fontWeight: 900,
                    cursor:
                      buying || (!course.is_free && !piUser) ? "not-allowed" : "pointer",
                  }}
                >
                  {course.is_free
                    ? "ابدأ الآن"
