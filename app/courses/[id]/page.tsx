...
export default function CourseDetailsPage() {
    ...
    const loadProtectedContent = async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error(
            json?.error ||
              (lang === "ar" ? "فشل تحميل محتوى الدورة" : "Failed to load course content")
          )
        }
  ...
  const handleBuyCourse = async () => {
    try {
      ...
      if (course.is_free) {
        ...
        setHasAccess(true)
        setStatus(
          lang === "ar" ? "تم فتح الدورة المجانية بنجاح" : "Free course unlocked successfully"
        )
        return
      }
  ...
  const handleDeleteCourse = async () => {
    try {
      ...
      setStatus(lang === "ar" ? "تم حذف الدورة بنجاح" : "Course deleted successfully")
  ...
  const hasWrittenContent =
    course.content_type === "article" && Array.isArray(articleBlocks) && articleBlocks.length > 0
            ...
            {checkingAccess ? (
              ...
              <div style={purchaseAreaStyle}>
                ...
                <button
                  ...
                  disabled={buying || !piUser || (!course.is_free && !piReady)}
                  style={{
                    ...buyButtonStyle,
                    background:
                      buying || !piUser || (!course.is_free && !piReady)
                        ? "#94a3b8"
                        : "#0f172a",
                    cursor:
                      buying || !piUser || (!course.is_free && !piReady)
              ...
              <div style={accessAreaStyle}>
                ...
                {loadingProtectedContent ? (
                  <div style={contentBoxStyle}>
                    {lang === "ar" ? "جاري تحميل المحتوى المحمي..." : "Loading protected content..."}
                  </div>
                ) : null}
                ...
                {protectedContent?.video_url ? (
                  <div style={contentBoxStyle}>
                    ...
                    <video controls src={protectedContent.video_url} style={videoStyle} />
                  </div>
                ) : null}
