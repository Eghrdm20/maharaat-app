  ...
  useEffect(() => {
    const loadVisits = async () => {
      try {
        ...
        const rawUser = window.localStorage.getItem("pi_user")
        const sessionToken = window.localStorage.getItem("pi_session_token") || ""

        if (!rawUser || !sessionToken) {
          throw new Error("يجب تسجيل الدخول من صفحة الملف الشخصي أولًا")
        }

        let piUser: PiUser | null = null

        try {
          piUser = JSON.parse(rawUser)
        } catch (error) {
          console.error(error)
        }
      ...
      <div style={containerStyle}>
        <div style={headerStyle}>
          <div>
            ...
            <p style={subtitleStyle}>تعرض آخر الزيارات داخل التطبيق</p>
          </div>
        </div>
          ...
          <div style={listStyle}>
            {filteredVisits.map((visit) => (
              <div key={visit.id} style={visitCardStyle}>
                ...
                <div style={metaStyle}>
                  <strong>الاسم:</strong> {visit.username || "زائر بدون تسجيل"}
                </div>
