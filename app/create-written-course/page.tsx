...
export default function CreateWrittenCoursePage() {
  ...
  const updateTextHeading = (id: string, heading: string) => {
    setBlocks((prev) =>
      prev.map((block) =>
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const nonEmptyBlocks = blocks.filter((block) => {
      if (block.type === "text") {
        return (
          block.heading.trim().length > 0 || block.text.trim().length > 0
        )
      }
      return !!block.file
    })
    ...
    <main style={{ minHeight: "100vh", paddingBottom: 40 }}>
      <div style={{ maxWidth: 700, margin: "0 auto", padding: 14 }}>
        ...
        <p style={subtitleStyle}>
          هذه الدورة تُقرأ داخل الصفحة فقط، ويمكنك إضافة عناوين وفقرات وصور.
        </p>

        {!piUser ? (
          <div style={infoBoxStyle}>
            يجب ربط حساب Pi أولًا من صفحة الملف الشخصي.
          </div>
        ) : (
                  ...
                  {block.type === "text" ? (
                    <div style={{ display: "grid", gap: 10 }}>
                      <input
                        value={block.heading}
                        onChange={(e) =>
                          updateTextHeading(block.id, e.target.value)
                        }
